---
title: "Practical Exam Bagilidad"
...
#ans. 
#The breaks columns of warpbreaks are numeric. 
#The wool columns of warpbreaks are factor.
#The tension columns of warpbreaks are factor. 
```{r}
str(warpbreaks)
```
...
#ans. There are total of 54 observations.
```{r}
...
```{r}
# When R returns an object's underlying type rather than its user-level class, it indicates that the expected and real data types are not the same.
